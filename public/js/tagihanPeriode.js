var dropdownTagihanMaster = "";
var dataTagihanMaster = "";

$(document).ready(function () {
    $(".dtSemester .dataTable-dropdown label").before("<button class='btn btn-primary' id='addData'>Tambah Data </button>  ");
    for (i = 0; i < dtTagihanMaster.length; ++i) {
        dataTagihanMaster += `<option value="${dtTagihanMaster[i]["id"]}">${dtTagihanMaster[i]["nama"]}</option>`;
    }
    dropdownTagihanMaster = `<select class="form-control tagihan_id">${dataTagihanMaster} </select>`;
});
...
$(document).on("click", "#addData", function () {
    ...
    $("tbody").prepend(`
        <tr class="tr_${className}">
            <td>${dropdownTagihanMaster}</td>
            <td><input type='text' class="form-control nama" id=''></td>
            <td><input type='date' class="form-control jatuh_tempo" id='' value=0></td>
            <td>
...
function getData(tr){
    ...
    dataPost.id =  tr;
    dataPost.tagihan_id = $(`.tr_${tr} .tagihan_id`).val();
    dataPost.nama = $(`.tr_${tr} .nama`).val();
    dataPost.jatuh_tempo = $(`.tr_${tr} .jatuh_tempo`).val();
